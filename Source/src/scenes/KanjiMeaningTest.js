...
class KanjiMeaningTest extends Component {
  ...
  setupTestIteration(){
    ...
    for(let i = 0; i<4; i++){
      ...
      if(AnswersType=="eng"){
        ...
        let meanings = PossibleAnswers[RandomIntTemp].data.meaning;
    ...
    if(!this.props.IsPaused){
      return (
        <AppContainer>
         ...
         {this.props.QuestuonItem?<KanjiTestQuestion test="Meaning" answersType={this.props.AnswersType} item={this.props.QuestuonItem} />:null} 
...
export default connect(mapStateToProps, mapDispatchToProps)(KanjiMeaningTest);
